...
if ($verified_status === 'TXN_SUCCESS' && $txn_status === 'TXN_SUCCESS') {
    ...
    // Refresh donation row to get updated_at timestamp
    $donation = $db->fetch("SELECT * FROM donations WHERE transaction_id=? LIMIT 1", [$order_id]);
    $donation['payment_mode'] = $payment_mode;

    // 2. Fetch donor user record
    $user = [];
    if (!empty($donation['user_id'])) {
        $user = $db->fetch("SELECT id, name, email, phone, address, pan_number FROM users WHERE id=? LIMIT 1",
            [$donation['user_id']]) ?? [];
    }

    // If guest donation (no user_id), use fields stored on the donation row
    if (empty($user)) {
        $user = [
